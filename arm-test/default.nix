...
, bcc-node-info ? sources.bcc-node
, bcc-node-src ? bcc-node-info
, bcc-wallet-src ? sources.bcc-wallet
, ogmios-src ? sources.ogmios
, bech32-src ? sources.bech32
# , bcc-rt-view-json
# , bcc-rt-view-info ? __fromJSON (__readFile bcc-rt-view-json)
# , bcc-rt-view-src ? nativePkgs.fetchgit (removeAttrs bcc-rt-view-info [ "date" ])
# , wstunnel-json
...
nativePkgs.lib.mapAttrs (_: pkgs: rec {
  ...
  __ogmios = (pkgs.haskell-nix.cabalProject {
    ...
    modules = [
      {
        packages.unix-bytestring.patches = [ ./bcc-node-patches/unix-bytestring-0.3.7.3.patch ];
        packages.terminal-size.patches = [ ./bcc-node-patches/terminal-size-0.3.2.1.patch ];
        packages.plutus-core.patches = [ ./bcc-node-patches/plutus-core.patch ];
      }
    ];
  });
  ...
  bcc-node = nativePkgs.lib.mapAttrs (_: bcc-node-info:
    let bcc-node-src = bcc-node-info; in rec {
    __bcc-node = (pkgs.haskell-nix.cabalProject {
        cabalProjectLocal  =  pkgs.lib.optionalString (pkgs.stdenv.targetPlatform != pkgs.stdenv.buildPlatform) ''
          ...
          '';
        compiler-nix-name = haskellCompiler;
        # pkgs.haskell-nix.haskellLib.cleanGit { name = "bcc-node"; src = ... } <- this doesn't work with fetchgit results
        src = bcc-node-src;
        # ghc = pkgs.buildPackages.pkgs.haskell-nix.compiler.${haskellCompiler};
        modules = [
          ...
          { doHaddock = false; }
          { compiler.nix-name = haskellCompiler; }
          { packages.bcc-config.flags.systemd = false;
            packages.bcc-node.flags.systemd = false; }
          { packages.terminal-size.patches = [ ./bcc-node-patches/terminal-size-0.3.2.1.patch ];
            packages.unix-bytestring.patches = [ ./bcc-node-patches/unix-bytestring-0.3.7.3.patch ];
            packages.plutus-core.patches = [ ./bcc-node-patches/plutus-core.patch ];

            # We need the following patch to work around this grat failure :(
            # src/Bcc/Config/Git/Rev.hs:33:35: error:
            ...
            #    |
            packages.bcc-config.patches = [ ./bcc-node-patches/bcc-config-no-git-rev.patch ];
            # packages.typerep-map.patches = [ ./bcc-node-patches/typerep-map-PR82.patch ];
            # packages.streaming-bytestring.patches = [ ./bcc-node-patches/streaming-bytestring-0.1.6.patch ];
            # packages.cole-spec-ledger.patches = [ ./bcc-node-patches/cole-ledger-spec-no-goblins.patch ];
            packages.cole-spec-ledger.flags.goblins = false;
            # this one will disable gitRev; which fails (due to a linker bug) for armv7
            # packages.bcc-config.patches = [ ./bcc-node-patches/1036.patch ];
          ...
          ({ pkgs, lib, ... }: lib.mkIf (pkgs.stdenv.hostPlatform.isAndroid) {
            packages.tbco-monitoring.patches = [ ./bcc-node-patches/tbco-monitoring-framework-625.diff ];
            # android default inlining threshold seems to be too high for closure_sizeW to be inlined properly.
            packages.bcc-prelude.ghcOptions = [ "-optc=-mllvm" "-optc-inlinehint-threshold=500" ];
            packages.bcc-node.ghcOptions = [ "-pie" ];
          })
          ({ pkgs, lib, ... }: lib.mkIf (!pkgs.stdenv.hostPlatform.isGhcjs) {
            packages = {
              # See https://github.com/the-blockchain-company/tbco-nix/pull/488
              bcc-crypto-praos.components.library.pkgconfig = lib.mkForce [ [ pkgs.libsodium-vrf ] ];
              bcc-crypto-class.components.library.pkgconfig = lib.mkForce [ [ pkgs.libsodium-vrf ] ];
            };
          })
          ({ pkgs, lib, ... }: lib.mkIf (pkgs.stdenv.hostPlatform.isGhcjs) {
            packages =
          ...
          # {
          #   packages.bcc-node-capi.components.library = {
          #   };
          # }
        ];
      });
      ...
      __bcc-wallet = (pkgs.haskell-nix.cabalProject {
        cabalProjectLocal  =  pkgs.lib.optionalString (pkgs.stdenv.targetPlatform != pkgs.stdenv.buildPlatform) ''
          -- When cross compiling we don't have a `ghc` package
          package plutus-tx-plugin
            flags: +use-ghc-stub
        ...
        modules = [
          ...
          { packages.terminal-size.patches = [ ./bcc-node-patches/terminal-size-0.3.2.1.patch ];
            packages.unix-bytestring.patches = [ ./bcc-node-patches/unix-bytestring-0.3.7.3.patch ];
            packages.plutus-core.patches = [ ./bcc-node-patches/plutus-core.patch ];
            packages.scrypt.patches = [ ./bcc-wallet-patches/scrypt-0.5.0.patch ];
          }
          {
            # Disable cabal-doctest tests by turning off custom setups
            packages.comonad.package.buildType = nativePkgs.lib.mkForce "Simple";
      ...
      inherit (__bcc-wallet.bcc-wallet.components.exes) bcc-wallet;
      inherit (__bcc-node.bcc-node.components.exes) bcc-node;
      inherit (__bcc-node.bcc-cli.components.exes)  bcc-cli;
      inherit (__bcc-node.bcc-submit-api.components.exes) bcc-submit-api;

      bcc-node-capi = __bcc-node.bcc-node-capi.components.library.override {
              smallAddressSpace = true; enableShared = false;
              ghcOptions = [ "-shared" "-v" "-optl-Wl,--version-script=${nativePkgs.writeText "libbcc-node-capi.version" ''
              CN_CAPI_1.0 {
                global:
                  hs_init;
                  setLineBuffering;
              ...
              postInstall = ''
                ...
                cp a.out $out/bcc-node-capi.so
                ${nativePkgs.patchelf}/bin/patchelf --set-soname "bcc-node-capi.so" $out/bcc-node-capi.so
                for x in $(${nativePkgs.patchelf}/bin/patchelf --print-needed $out/bcc-node-capi.so |grep -v "so$"|xargs); do
                  ${nativePkgs.patchelf}/bin/patchelf --replace-needed $x ''${x%%.so.*}.so $out/bcc-node-capi.so;
                done
      ...
      tarball = nativePkgs.stdenv.mkDerivation {
        ...
        buildPhase = ''
          mkdir -p bcc-node
          cp ${bcc-cli}/bin/*bcc-cli* bcc-node/
          cp ${bcc-node.override { enableTSanRTS = false; }}/bin/*bcc-node* bcc-node/
          cp ${bcc-submit-api}/bin/*bcc-submit* bcc-node/
          cp ${bech32}/bin/bech* bcc-node/
        '' + pkgs.lib.optionalString (pkgs.stdenv.targetPlatform.isLinux && pkgs.stdenv.targetPlatform.isGnu) ''
          for bin in bcc-node/*; do
            mode=$(stat -c%a $bin)
            chmod +w $bin
            patchelf --set-interpreter /lib/ld-linux-armhf.so.3 $bin
            chmod $mode $bin
        ...
        '' + pkgs.lib.optionalString (pkgs.stdenv.targetPlatform.isWindows) ''
          ...
          cp ${pkgs.libffi}/bin/*.dll bcc-node/
          ${nativePkgs.tree}/bin/tree ${pkgs.gmp}
          cp ${pkgs.gmp}/bin/*.dll* bcc-node/
          ${nativePkgs.tree}/bin/tree ${pkgs.zlib}
          cp ${pkgs.zlib}/bin/*.dll* bcc-node/
        '' + pkgs.lib.optionalString (pkgs.stdenv.targetPlatform.isLinux && pkgs.stdenv.targetPlatform.isGnu) ''
          cp ${pkgs.libffi}/lib/*.so* bcc-node/
          cp ${pkgs.gmp}/lib/*.so* bcc-node/
          cp ${pkgs.ncurses}/lib/*.so* bcc-node/
          cp ${pkgs.zlib}/lib/*.so* bcc-node/
          echo ${pkgs.stdenv.cc}/lib
          ls bcc-node/
        '';
        installPhase = ''
          mkdir -p $out/
          zip -r -9 $out/${pkgs.stdenv.hostPlatform.config}-bcc-node-${bcc-node-info.rev or "unknown"}.zip bcc-node
      ...
      bcc-node-spk = (mkSynologySpk {
        name = "bcc-node";
        version = bcc-node-info.rev or "unknown";
        pkg = bcc-node;
        run = ''
          #!/bin/sh

          /var/packages/bcc-node/target/bin/bcc-node run \
            --topology /var/packages/bcc-node/target/etc/mainnet-topology.json \
            --config /var/packages/bcc-node/target/etc/mainnet-config.json \
            --port 3001 \
            --host-addr 0.0.0.0 \
            --database-path /var/packages/bcc-node/var/db \
            --socket-path /var/packages/bcc-node/tmp/node.socket
          '';
      }).spk;

      bcc-submit-api-spk = (mkSynologySpk {
        name = "bcc-submit-api";
        version = bcc-node-info.rev or "unknown";
        pkg = bcc-submit-api;
        run = ''
          ...
          /var/packages/bcc-submit-api/target/bin/bcc-submit-api \
            --socket-path /var/packages/bcc-node/tmp/node.socket \
            --config /var/packages/bcc-submit-api/target/etc/tx-submit-mainnet-config.yaml \
            --port 8090 \
          ...
          '';
            ...
            # cp ${bcc-cli}/bin/*bcc-cli*            bin
            # cp ${bcc-node}/bin/*bcc-node*          bin
            # cp ${bcc-submit-api}/bin/*bcc-submit*  bin


    }) { "mainnet" = sources.bcc-node-mainnet; "vasil" = sources.bcc-node-vasil; };
